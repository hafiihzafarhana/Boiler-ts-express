...
class TokenMiddleware {
  public authenticate: RequestHandler<ParamsDictionary, unknown, unknown, ParsedQs, Record<string, unknown>> = asyncHandler(
    async (req: AuthorizeRequest, _res: Response, next: NextFunction): Promise<void> => {
      ...
      if (decodedAccessToken) {
        ...
        if (req.user) {
          req.user.acc_token = accToken as string;
        }
      ...
      const newAccessToken = generateAccessToken({ user_id: decodedRefreshToken.user_id as string });
      ...
      req.user = { user_id: decodedRefreshToken.user_id as string, acc_token: newAccessToken };
